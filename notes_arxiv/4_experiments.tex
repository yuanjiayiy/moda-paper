...
The training set contains 10K prompts, constructed from a 4:1 mixture of uniformly sampled Tulu3-SFT-Mixture~\citep{lambert2024tulu3} and Infinite-Chat dataset~\citep{jiang2025artificialhivemindopenendedhomogeneity}. This mixture balances quality and diversity: Tulu3-SFT-Mixture provides instruction-following prompts that preserve response quality, while Infinite-Chat encourages open-ended exploration.
...
We observe several interesting patterns in our experiments. Consistent with the quantitative results, \method{} produces a larger set of valid and distinct responses.  Figure~\ref{fig:moda + 40blue} visualizes responses to the prompt \textbf{``Name a shade of blue''} from Qwen3-8B, Qwen3-8B with \ssot, and \method{} with thinking disabled. Figure~\ref{fig:qualitative-example-books} shows that, when asked to \textbf{name a well-received book}, the base model produces the same answer, ``To Kill a Mockingbird,'' in all six samples, whereas \method{} produces six distinct titles; Appendix~\ref{appx: generation examples} Figure \ref{fig:time} compares responses to the prompt \textbf{``Write a metaphor about time''} from 23 off-the-shelf models and from \method{}. The baseline models converge almost entirely on the metaphor ``time is a river,'' whereas \method{} produces a diverse set of analogies. Additional qualitative examples are provided in Appendix~\ref{appx: generation examples}. Although \method{} uses only abstract numbered roles rather than hand-crafted personas, the model sometimes learns role-specific behavioral patterns, suggesting that abstract mode conditioning can induce differentiated characteristics. 
